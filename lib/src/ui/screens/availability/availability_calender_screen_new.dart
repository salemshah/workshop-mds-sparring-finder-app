...
class AvailabilityCalendarScreenNew extends StatefulWidget {
  const AvailabilityCalendarScreenNew({super.key});

  @override
  State<AvailabilityCalendarScreenNew> createState() =>
      _AvailabilityCalendarScreenNewState();
}

class _AvailabilityCalendarScreenNewState
    extends State<AvailabilityCalendarScreenNew> {
  final CalendarController _cal = CalendarController();

  /// All slots fetched from backend
  List<Availability> _availabilities = [];

  /// Syncfusion expects Windows‐style TZ names:
  static const _tzParis = 'Romance Standard Time';

  // Helpers to normalize between UTC (model) and local (UI)
      ...
      body: SafeArea(
        child: BlocConsumer<AvailabilityBloc, AvailabilityState>(
          ...
          builder: (context, state) {
            // 1) Build Availability appointments (green)
            final availAppts = _availabilities.map((a) {
              final localStart = _toLocal(a.startTime);
              final localEnd = _toLocal(a.endTime);
              return Appointment(
            ...
            // 2) Build Sparring appointments (colored by status)
              ...
              onTap: (details) async {
                ...
                if (existing != null) {
                  ...
                  await _showCreateDialog(
                      context, tapped, tapped.add(const Duration(hours: 6)));
                }
              },
            );
          },
